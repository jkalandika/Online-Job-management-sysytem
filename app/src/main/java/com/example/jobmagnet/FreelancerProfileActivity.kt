...
class FreelancerProfileActivity : AppCompatActivity() {
    ...
    private lateinit var jobsDescriptionTextView: TextView
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // Initialize views
        nameTextView = findViewById(R.id.company_name_text_view)
        ageTextView = findViewById(R.id.company_type_text_view)
        jobsDescriptionTextView = findViewById(R.id.description_text_view)
            ...
            override fun onDataChange(dataSnapshot: DataSnapshot) {
                if (dataSnapshot.exists()) {
                    for (userSnapshot in dataSnapshot.children) {
                        ...
                        displayCompanyProfile(freelancerProfile)
                    }
                } else {
    ...
    private fun displayCompanyProfile(companyProfile: FreelancerProfile?) {
        nameTextView.text = companyProfile?.fullName
        ageTextView.text = companyProfile?.age
        jobsDescriptionTextView.text = companyProfile?.interestedJobs
                                ...
                                for (userSnapshot in dataSnapshot.children) {
                                    userSnapshot.ref.removeValue().addOnCompleteListener { task ->
                                        if (task.isSuccessful) {
                                            Toast.makeText(this@FreelancerProfileActivity, "Account deleted successfully", Toast.LENGTH_SHORT).show()
                                            val intent = Intent(this@FreelancerProfileActivity, MainActivity::class.java)
                                            startActivity(intent)
                                            finish()
                                        } else {
                                            Toast.makeText(this@FreelancerProfileActivity, "Failed to delete the company profile", Toast.LENGTH_SHORT).show()
                                        }
                                    }
                                }
                            } else {
